...
let PetsService = exports.PetsService = (() => {
    let _classDecorators = [(0, common_1.Injectable)()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var PetsService = _classThis = class {
        ...
        async verifyUserExists(userEmail) {
            ...
            if (user.role !== 'owner') {
                throw new common_1.BadRequestException('User must be an owner to add a pet');
            }
            if (!user)
                throw new UserNotFoundException_1.UserNotFoundException();
        }
    };
    __setFunctionName(_classThis, "PetsService");
    (() => {
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name }, null, _classExtraInitializers);
        PetsService = _classThis = _classDescriptor.value;
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return PetsService = _classThis;
})();
